...
export default function WorksImages(props: WorkImageProps){
    ...
    useGSAP(() =>{
        gsap.set([imageOneRef.current, imageTwoRef.current, imageThreeRef.current], {opacity:1})
    ...
    return (
        <div ref={containerRef} className="flex flex-col gap-8 basis-8/12 md:order-1 order-2">
            <img ref={imageOneRef} src={image_1} className="rounded-4xl max-w-full" />
            <div className="flex flex-row gap-8">
                <img ref={imageTwoRef} src={image_2} className="w-1/2  rounded-4xl" />
                <img ref={imageThreeRef} src={image_3} className="w-1/2  rounded-4xl" />
            </div>
         </div>
    )
}